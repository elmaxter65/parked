...
import 'package:flutter_firebase_flutter_2/src/bloc/map_bloc/geolocation_bloc.dart';
import 'package:flutter_firebase_flutter_2/src/bloc/map_bloc/map_bloc.dart';
...
import 'package:flutter_firebase_flutter_2/src/util/colors.dart';
...
class _MapsState extends State<Maps> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> _onItemCarouselChange(Todo place, index) async {
      GoogleMapController controller = await _mapController.future;
      CameraPosition _newPos = CameraPosition(
        target: LatLng(double.parse(place.lat), double.parse(place.lng)),
        zoom: 14.5,
      );
                        ...
                        if (snapshotPlaces.data.length > 0) {
                          return GoogleMap(
                            ...
                            initialCameraPosition: CameraPosition(
                              target: LatLng(
                                  double.parse(snapshotPlaces.data[0].lat),
                                  double.parse(snapshotPlaces.data[0].lng)),
                              zoom: 13.0,
                            ),
                          );
                        } else {
